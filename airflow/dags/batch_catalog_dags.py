...
def _load_pipeline_specs():
    if control_db_enabled():
        try:
            specs = load_enabled_pipeline_specs_from_control_db()
            if specs:
...
for pipeline_spec in _load_pipeline_specs():
    try:
        ...
        dag = DAG(
            ...
            description=pipeline_spec.get(
                "description",
                f"Metadata-driven DAG for {pipeline_spec['name']}",
            ),
            ...
            default_args={
                ...
                "retry_delay": timedelta(
                    minutes=default_args_cfg.get("retry_delay_minutes", 1)
                ),
            },
            tags=dag_cfg.get("tags", ["metadata_driven", "airflow", "batch"]),
        )

        with dag:
            build_tasks_from_pipeline_spec(
                dag=dag,
        ...
        print(
            f"[batch_catalog_dags] Failed to build DAG for pipeline "
            f"'{pipeline_spec.get('name', '?')}': {exc}"
        )