...
def _apply_incremental_filter(
    *,
    ...
    """
    Optional incremental filtering for Gold jobs.

    Gold can be either:
      - full snapshot/curated recompute; or
      - incremental merge from a primary Silver/fact view.
...
def _view_list_from_spec(spec: dict[str, Any], *, default_alias: str) -> list[dict[str, Any]]:
    ...
    Official contract:
      spec.views = [
        {"alias": "orders", "path": "s3a://...", "format": "delta"},
        {"alias": "customers", "path": "s3a://...", "format": "delta"}
      ]
    ...
    raise ValueError("Gold job requires spec.source, spec.sources, or spec.views")


def load_gold_input_dataframe(
    *,
    spark: SparkSession,
    spec: dict[str, Any],
    target_path: str,
    ...
    """
    Gold layer purpose:
      business-level curation, joins, aggregations, and denormalization.

    Required/official transformation contract:
      spec.views + spec.sql.query

    Each view is registered as a Spark temp view using view.alias.
    The SQL query is job-specific and belongs in the catalog JSON, not in Python.

    Backward-compatible mode:
      If sql.query is missing, the first view/source is passed through and optional
      filters/select_map/derived_fields/quality_rules/dedupe are applied.
    """
    views = _view_list_from_spec(spec, default_alias="source")
    ...
    if not view_frames:
        raise ValueError("No Gold input view was loaded")
    ...
    if sql_query:
        gold_df = spark.sql(sql_query)
        input_count = primary_count if incremental_spec.get("enabled", False) else sum(
            int(df.count()) for df in view_frames.values()
        )
        return gold_df, input_count, last_watermark

    # Backward-compatible single-source pass-through path.
    first_alias = next(iter(view_frames))
    input_df = view_frames[first_alias]
    input_count = primary_count if incremental_spec.get("enabled", False) else int(input_df.count())

    return input_df, input_count, last_watermark


def build_gold_dataframe(input_df: DataFrame, spec: dict[str, Any]) -> DataFrame:
    """
    Apply optional post-SQL transformations.

    The preferred Gold contract is SQL-first. These rules are retained for
    final filters, projections, derived fields, quality checks, and dedupe.
    """
    return (
        input_df
...
def main() -> None:
    ...
    with control_run(context=context) as run_ctx:
        try:
            ...
            gold_input_df, input_count, last_watermark = load_gold_input_dataframe(
                spark=spark,
                spec=spec,
                target_path=target_path,
            )
            ...
            gold_df = build_gold_dataframe(gold_input_df, spec)
            output_count = int(gold_df.count())
            ...
            if output_count == 0:
                ...
                print(
                    f"[GOLD_SKIP_EMPTY_AFTER_CURATE] "
                    f"read={input_count} "
                    f"target={target_path}",
                    flush=True,
                )
